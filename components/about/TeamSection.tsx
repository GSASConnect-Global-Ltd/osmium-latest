...
import type { TeamMember } from "@/types/team"; // ✅ use the shared type
      ...
      <div className="container px-6 mx-auto max-w-7xl">
        <div className="text-center mb-16">
          <h2
            ...
            className="mb-12 text-center ppEditorial text-[28px] sm:text-[36px] md:text-[48px] leading-[36px] sm:leading-[44px] md:leading-[56px] text-foreground"
          >
            Meet the Team
          </h2>
          <p className="mt-4 text-muted-foreground max-w-2xl mx-auto font-[Satoshi] text-[16px] sm:text-[18px] leading-relaxed">
            The brilliant minds driving ORREL’s mission of innovation, sustainability, and technological excellence.
          </p>
        </div>
        ...
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
          {teamMembers.map((member, index) => (
            <div
              key={index}
              className="group flex flex-col items-center text-center bg-card dark:bg-muted rounded-2xl p-6 hover:bg-muted/60 transition duration-500 transform hover:-translate-y-2 hover:scale-[1.03]"
            >
              ...
              {member.socials.length > 0 && (
                <div className="flex justify-center gap-4 mt-auto">
                  {member.socials.map((social, i) => (
                    ...
                    >
                      {/* Here you can optionally map social.platform to icons */}
                      {social.platform}
                    </a>
                  ))}
                </div>
